...
export const getExpenses = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const cacheKey = cacheService.generateKey('expenses', userId, JSON.stringify(req.query));
  ...
  try {
    ...
    logger.error('Get expenses query failed:', error);
    throw error;
  }
});
  ...
  try {
    ...
    if (validatedData.categoryId && validatedData.categoryId !== existingExpense.categoryId) {
      ...
      if (existingExpense.aiConfidence && Number(existingExpense.aiConfidence) > 0.1) {
        ...
        await jobQueue.add('learn-from-correction', {
  ...
  try {
    ...
    for (const [batchIndex, batch] of batches.entries()) {
      ...
      try {
        ...
        logger.warn(`Batch ${batchIndex} failed, trying individual inserts:`, batchError);
...
export const getExpenseStats = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const cacheKey = cacheService.generateKey('expense-stats', userId, period as string);
  ...
  try {
    ...
    logger.error('Get expense stats failed:', error);
    throw error;
  }
});
...
export const getRecurringExpenses = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const cacheKey = cacheService.generateKey('recurring-expenses', userId);
  ...
  try {
    ...
    logger.error('Get recurring expenses failed:', error);
    throw error;
  }
});
...
export const getCategories = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const categoriesWithStats = await prisma.$queryRaw<Array<{
      id: string;
      name: string;
      description: string;
      color: string;
...
export const getExpenseTags = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const cacheKey = cacheService.generateKey('expense-tags', userId);
  ...
  try {
    ...
    const tagsRaw = await prisma.$queryRaw<Array<{ 
      tag: string; 
      usage_count: number;
      last_used: Date;
    }>>`
    ...
    logger.error('Get expense tags failed:', error);
    throw error;
  }
});
...
export const categorizeExpense = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const job = await jobQueue.add('categorize-expense', {
      expenseId: expense.id,
      expenseData: {
    ...
    logger.error('Categorize expense failed:', error);
    throw error;
  }
});
...
export const bulkRecategorize = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const job = await jobQueue.add('bulk-recategorize', {
      userId,
      limit: parsedLimit,
      onlyLowConfidence,
      categoryIds,
    ...
    logger.error('Bulk recategorize failed:', error);
    throw error;
  }
});
...
export const getExpenseById = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const cacheKey = cacheService.generateKey('expense', userId, id);
...
export const getExpenseInsights = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const { timeframe = '30' } = req.query; // days
  
  const cacheKey = cacheService.generateKey('expense-insights', userId, timeframe as string);
  ...
  try {
    ...
    logger.error('Get expense insights failed:', error);
    throw error;
  }
});

// Search expenses with optimized full-text search
export const searchExpenses = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const cacheKey = cacheService.generateKey('expense-search', userId, searchTerm, limit as string);
  ...
  try {
    ...
    logger.error('Search expenses failed:', error);
    throw error;
  }
});
...
async function getOrCreateDefaultCategory() {
  ...
  let defaultCategory = await cacheService.get<any>(cacheKey);
  if (defaultCategory) {
  ...
  // Cache for 1 hour
  await cacheService.set(cacheKey, defaultCategory, { ttl: 3600 });
  return defaultCategory;
}
...
async function getJobQueueStats() {
  try {
    ...
    logger.error('Failed to get job queue stats:', error);
    return null;
  }
}